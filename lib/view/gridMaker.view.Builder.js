			...
			,add : function(){
				...
				if (CAIRS.dhtmlx.validateForm("gridMaker_Builder_form", self.form)) {
					...
					*/
					...
					///localStorage.setItem(that.settings.appId + "_gridSettings_" + self.container, JSON.stringify(that.savedSettings[ self.container ]));
		...
		,grid : {
						...
						CAIRS.MAP.API.post({
							...
							,onSuccess : function( request )
							{
								...
								localStorage.setItem(	
									that.settings.appId + "_gridSettings_" + self.container, 
									JSON.stringify(that.savedSettings[ self.container ]	)
								);
								
								self.layout.progressOff();
								
								self.helpers.grid.columns.save( self.configuration[self.container ].fnCallBack );
							}
			...
			,columns : {
					...
					CAIRS.MAP.API.post({
						...
						,onSuccess : function( request )
						{
							try{
								...
								var storageName = that.settings.appId + "_gridSettings_" + self.container;
								localStorage.setItem(	
									storageName, 
									JSON.stringify( that.savedSettings[ self.container ] )	
								);
								
								if( fnCallBack ) fnCallBack( JSON.parse( localStorage.getItem( storageName )) );
				...
				,del: function(  ){
					...
					/*that.savedSettings[ self.container ].grid.forEach(function (column, index, array_) {
