gridMaker.view.Builder = {
	...
	_window: function ( ) {
		...
		self.window.attachEvent("onClose", function(win){
			...
			if( self.tableAlreadyCreated == false )
				if( self.configuration[ self.field_id ].fnCallBack ) self.configuration[ self.field_id ].fnCallBack( null, false );
			...
			,add : function(){
				...
				if (CAIRS.dhtmlx.validateForm("gridMaker_Builder_form", self.form)) {
					...
					*/
					...
					// old style grid
					that.savedSettings[ self.field_id ].grid.headers.push( hash["dhtmlx_grid_header"] );
					that.savedSettings[ self.field_id ].grid.ids.push( hash["column_name"] );
					that.savedSettings[ self.field_id ].grid.widths.push( hash["dhtmlx_grid_width"] );
					that.savedSettings[ self.field_id ].grid.colaligns.push( hash["dhtmlx_grid_align"] );
					that.savedSettings[ self.field_id ].grid.coltypes.push( hash["dhtmlx_grid_type"] );
					that.savedSettings[ self.field_id ].grid.colsorting.push( hash["dhtmlx_grid_sorting"] );
					
					that.savedSettings[ self.field_id ].columns.push( hash );
					
					// new set up approach for grid
					/*that.savedSettings[ self.field_id ].grid.head.push( {
						id: hash["column_name"],    
					...
					///localStorage.setItem(that.settings.appId + "_gridSettings_" + that.configuration.field_id, JSON.stringify(that.savedSettings[ self.field_id ]));
		...
		,grid : {
					...
					if( ! self.tableAlreadyCreated && self.teableCreatedID == -1 )
					{
						CAIRS.MAP.API.post({
							...
							,onSuccess : function( request )
							{
								...
								that.savedSettings[ self.field_id ].grid_name = grid_name_input.value;
								that.savedSettings[ self.field_id ].table_name = self.tableName;
								that.savedSettings[ self.field_id ].gridmaker_table_id = self.teableCreatedID;
								
								
								localStorage.setItem(	
									that.settings.appId + "_gridSettings_" + that.configuration.field_id, 
									JSON.stringify(that.savedSettings[ self.field_id ]	)
								);
								
								self.layout.progressOff();
								
								self.helpers.grid.columns.save( self.configuration[self.field_id ].fnCallBack );
							}
			...
			,columns : {
				...
				,save: function( fnCallBack ){
					...
					that.savedSettings[ self.field_id ].columns.forEach(function (column, index, array_) {
						that.savedSettings[ self.field_id ].columns[ index ][ "gridmaker_table_id" ] =  self.teableCreatedID;
					});
					
					CAIRS.MAP.API.post({
						resource :  "/gridmaker/builder/tables/" + self.teableCreatedID + "/columns"
						,format : "json" // not mandatory, default json
						,payload : "table_name="+that.savedSettings[ self.field_id ].table_name+"&table_id="+self.teableCreatedID+"&hash=" + JSON.stringify( that.savedSettings[ self.field_id ].columns )
						,onSuccess : function( request )
						{
							try{
								var json = JSON.parse( request.response );
								...
								if( fnCallBack ) fnCallBack( JSON.parse( localStorage.getItem( storageName )), true );
						...
						{
							var json = JSON.parse( request.response );
							if( fnCallBack ) fnCallBack( null, false );
						}
					});
				...
				,del: function(  ){
					...
					that.savedSettings[ self.field_id ].columns.forEach(function (column, index, array_) {
						if( column.column_name == column_name ) that.savedSettings[ self.field_id ].columns.splice(index, 1);
					});
						
					/*that.savedSettings[ self.field_id ].grid.forEach(function (column, index, array_) {
						if( column.id == column_name ) that.savedSettings[ self.field_id ].grid.head.splice(index, 1);
					});*/
	...
	,render : function( configuration ){
		...
		self.field_id = configuration.field_id;
		
		
		self.configuration[self.field_id ] = configuration;
		
		try
		{
			if(! self.isOpened)
			{
				that.addNewSettings( self.field_id );
